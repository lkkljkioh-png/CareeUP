...
async function loadFavoriteGraduates() {
    ...
    try {
        ...
        if (!response.ok || !result.success) {
            throw new Error(
                result.message || "관심 졸업생 조회 실패"
            );
        }

        renderFavoriteGraduates(result.data || []);

    } catch (error) {
        console.error("관심 졸업생 조회 오류:", error);
    }
}

// 관심 졸업생 목록 출력
function renderFavoriteGraduates(graduates) {
    const container =
        document.getElementById("favorite-graduate-list");

    const count =
        document.getElementById("favorite-graduate-count");

    if (!container || !count) {
        return;
    }

    count.textContent = graduates.length;
    ...
    graduates.forEach(graduate => {
        ...
        item.className = "favorite-graduate-item";

        const info = document.createElement("div");

        info.className = "favorite-graduate-info";
        ...
        detailButton.className = "favorite-detail-btn";
        detailButton.textContent = "프로필 보기";
...
// 최근 본 졸업생 개수 표시
function loadRecentGraduateCount() {
    const countElement =
        document.getElementById("recent-graduate-count");

    if (!countElement) {
        console.error(
            "recent-graduate-count 요소를 찾을 수 없습니다."
        );
        return;
    }
    ...
    try {
        const saved =
            localStorage.getItem("recentGraduates");

        if (saved) {
            const parsed = JSON.parse(saved);

            if (Array.isArray(parsed)) {
                recentGraduates = parsed;
            }
        }
